...
function databasePath() {
  return (
    process.env.INVARIANCE_DB_PATH ??
    path.join(process.cwd(), ".data", "invariance.sqlite")
  );
}
...
function getDatabaseSyncConstructor(): DatabaseSyncConstructor {
  ...
  try {
        ...
        const type =
          typeof args[0] === "string"
            ...
            : args[0] && typeof args[0] === "object" && "type" in args[0]
              ...
              : undefined;
        if (type === "ExperimentalWarning" && /SQLite/i.test(warningText))
          return;
        return originalEmitWarning.call(
          process,
          warning as string,
          ...(args as []),
        );
    ...
    const message =
      error instanceof Error ? error.message : "node_sqlite_unavailable";
    throw new Error(
      `SQLite runtime unavailable. Use DATABASE_PROVIDER=postgres or run on a Node version with node:sqlite. ${message}`,
    );
  }
}
...
export function getDb() {
  if (getDatabaseProvider() === "postgres") {
    throw new Error(
      "SQLite getDb() was called while DATABASE_PROVIDER=postgres. Use repository/provider contracts instead.",
    );
  }
  if (db) return db;
  ensureDbDir();
