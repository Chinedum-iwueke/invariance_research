...
export const exportQueue = {
  async enqueueRun(exportId: string) {
    await exportJobRepository.updateByExportId(exportId, (current) => ({ ...current, status: "queued", available_at: new Date().toISOString() }));
    if (shouldRunEmbeddedWorkers()) startExportWorker();
  },
  async enqueueRetry(exportId: string, retryCount: number) {
    const availableAt = new Date(Date.now() + BASE_BACKOFF_MS * Math.max(1, retryCount)).toISOString();
    await exportJobRepository.updateByExportId(exportId, (current) => ({
      ...current,
      status: "queued",
