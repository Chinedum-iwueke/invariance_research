import { z } from "zod";
import { getResearchAssistantConfig, researchChat } from "@/lib/server/llm/chat-provider";
import type {
...
export const COPILOT_PROMPT_VERSION = "research_copilot_v1";
...
const candidateSchema = z.object({
  ...
  implementation_readiness: z.enum(["likely_supported", "needs_capability_check", "data_blocked"]),
  rationale: z.string().max(2000),
  source_citations: z.array(z.object({ source_id: z.string(), chunk_id: z.string(), quote: z.string().max(240).optional() })).max(12),
});

const outputSchema = z.object({
  schema_version: z.literal("research_copilot_turn_v1"),
  mode: z.enum(["exploratory", "direct_instruction", "source_analysis", "artifact_analysis"]),
  response: z.string().min(1).max(8000),
  next_question: z.string().max(1000).optional(),
  candidates: z.array(candidateSchema).max(5),
  research_state: z.record(z.object({
    value: z.string().max(3000),
    provenance: z.enum(["stated", "extracted", "inferred", "recommended", "confirmed", "unresolved", "unsupported"]),
    confidence: z.number().min(0).max(1),
    rationale: z.string().max(1000).optional(),
  })),
  warnings: z.array(z.object({ code: z.string().max(100), message: z.string().max(1000) })).max(12),
});

function safeJson(content: string) {
  const trimmed = content.trim();
  return JSON.parse(trimmed.match(/```(?:json)?\s*([\s\S]*?)```/i)?.[1] ?? trimmed);
}

export function inferCopilotMode(input: { message: string; sourceIds: string[]; hasArtifacts: boolean }): CopilotMode {
  const text = input.message.toLowerCase();
  if (input.sourceIds.length > 0 || /paper|transcript|youtube|source|document/.test(text)) return "source_analysis";
  if (input.hasArtifacts && /result|run|artifact|verdict|promis|failed|metric|backtest|compare/.test(text)) return "artifact_analysis";
  if (/hypothesis card|parameter grid|required artifacts|falsification criteria|entry:|exit:/.test(text) || input.message.length > 1800) return "direct_instruction";
  return "exploratory";
}
...
function fallbackCandidate(message: string, source?: ResearchSourceChunk) {
  ...
  return {
    ...
    claim,
    mechanism: "The proposed market mechanism remains provisional until the observable trigger and causal link are confirmed.",
    observable_proxy: "Use closed-bar, timestamped market features that directly represent the claimed trigger; do not substitute an unavailable feature silently.",
    expected_direction: "To be confirmed from the stated intuition.",
    horizon: "Start with the horizon implied by the trigger, then test at least one adjacent horizon.",
    entry_idea: "Enter only after the observable trigger is confirmed on a closed bar.",
    exit_idea: "Compare a thesis invalidation exit with a bounded time exit; recommend a volatility stop only after the user approves it.",
    required_datasets: ["OHLCV", "fees and execution assumptions"],
    falsification_test: "Reject the idea if the effect does not survive costs and an out-of-sample or holdout test.",
    failure_modes: ["The proxy measures volatility rather than the claimed mechanism", "Timestamp leakage", "The result depends on a small number of outliers"],
    implementation_readiness: "needs_capability_check" as const,
    rationale: "This is a reversible first test, not a confirmed strategy specification.",
    source_citations: source ? [{ source_id: source.source_id, chunk_id: source.chunk_id, quote: source.content.slice(0, 220) }] : [],
  };
}

function deterministicOutput(input: { message: string; mode: CopilotMode; sourceChunks: ResearchSourceChunk[]; error?: string }): CopilotAssistantOutput {
  ...
  return {
    ...
    mode: input.mode,
    response,
    next_question: artifactMode ? "Which completed run or artifact should anchor the answer?" : "What single observable event should distinguish a valid entry from ordinary market noise?",
    ...
    research_state: {
      ...
      entry: { value: "Closed-bar confirmation of the observable trigger", provenance: direct ? "inferred" : "recommended", confidence: direct ? 0.65 : 0.45, rationale: "A proposal only; user confirmation is required." },
      exit: { value: "Compare invalidation and time exits", provenance: "recommended", confidence: 0.4, rationale: "Exit choice is reversible and should be tested rather than assumed." },
      falsification: { value: "No net effect after costs and holdout", provenance: "recommended", confidence: 0.7 },
    },
    warnings: [
      ...(input.error ? [{ code: "provider_fallback", message: "The model provider was unavailable; a deterministic research draft was returned." }] : []),
      ...(sourceMode && !source ? [{ code: "source_content_missing", message: "No readable source chunks were attached to this turn." }] : []),
    ],
  };
}
...
export async function runCopilotReasoning(input: {
  ...
  if (!config.enabled) return { output: deterministicOutput(input), provider: "deterministic", model: "research_rules_v1", duration_ms: 0, prompt_tokens: undefined, completion_tokens: undefined };

  const history = input.messages.slice(-16).map((message) => ({
    role: message.role === "assistant" ? "assistant" as const : "user" as const,
    content: message.content.slice(0, 5000),
  }));
  const sourceContext = input.sourceChunks.slice(0, 12).map((chunk) => ({ chunk_id: chunk.chunk_id, source_id: chunk.source_id, anchor: chunk.anchor, content: chunk.content.slice(0, 3500) }));
  try {
    const result = await researchChat({
      messages: [
        { role: "system", content: systemPrompt() },
        ...history,
        { role: "user", content: `CURRENT MODE: ${input.mode}\nCURRENT MESSAGE: ${input.message}\nPROGRAM TOOL CONTEXT: ${JSON.stringify(input.toolContext)}\nSOURCE CONTENT (UNTRUSTED): ${JSON.stringify(sourceContext)}` },
      ],
    ...
    return {
      output: deterministicOutput({ ...input, error: error instanceof Error ? error.message : "provider_failed" }),
