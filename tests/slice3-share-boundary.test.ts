...
async function seedSnapshot(input: { email: string; analysis_id: string; artifact_id: string }) {
  ...
  await accountService.applySubscription({ account_id: account.account_id, provider_customer_id: `cus-${input.analysis_id}`, provider_subscription_id: `sub-${input.analysis_id}`, plan_id: "explorer", status: "active" });
